...
for i in range(10,20):
    ...
    with open('CoalCreekResults_v1.pkl', 'rb') as f:
        ...
        def export_to_csv(keys, data, name):
            with open(f"{name}_exported_062921_best10AIC.csv", "w+") as datafile:
...
# fig1.savefig('PhysicalParamsFigure_062921_medMC.png',dpi=200,bbox_inches='tight')
...
#     def export_to_csv(keys, data, name):
#         with open(f"{name}_exported_062921_medMC.csv", "w+") as datafile:
#             datafile.write(",".join(keys) + '\n')
#             numpy_stuff = np.asarray(data).T
#             for row in numpy_stuff:
#                 datafile.write(','.join([str(x) for x in row]) + '\n')
